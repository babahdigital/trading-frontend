...
export default function CryptoRiskPage() {
  const t = useTranslations('portal.crypto.risk');
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1.5 text-sm">
          {t('tagline_pre')} <span className="font-mono text-amber-300">{tier}</span> {t('tagline_post', { cap })} <span className="font-mono">{t('tagline_runtime')}</span>{t('tagline_period')}
        </p>
      </div>
      ...
      {success && (
        <div className="p-3 rounded-md border border-green-500/30 bg-green-500/5 text-green-300 text-sm flex items-center gap-2">
          <CheckCircle2 className="h-4 w-4" /> {t('save_success')}
        </div>
      )}
        ...
        <CardContent className="p-5 sm:p-6 space-y-5">
          <div>
            <div className="flex items-center justify-between gap-3 flex-wrap mb-1">
              <h2 className="font-semibold">{t('leverage_override')}</h2>
              <span className="text-[11px] font-mono uppercase tracking-wider text-muted-foreground">
                {t('effective_label', { value: state?.effective ?? override })}
              </span>
            </div>
          ...
          <div>
            <label htmlFor="leverage-input" className="block text-sm font-medium mb-1.5">{t('user_override')}</label>
          ...
          {aiSuggestion != null && (
            <div className="flex items-start gap-2.5 p-3 rounded-md bg-amber-500/5 border border-amber-500/20 text-xs text-amber-200/90">
              ...
              <div>
                <span className="font-semibold">{t('ai_suggestion_pre')}</span> {t('ai_suggestion_value', { value: aiSuggestion })} ·
                <button
                  type="button"
      ...
      <Card className="border-red-500/30 bg-red-500/[0.03]">
        <CardContent className="p-5 sm:p-6 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-bold text-red-300">{t('kill_heading')}</h2>
        ...
        <DialogContent className="border-red-500/40">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-red-300">
              <AlertOctagon className="h-5 w-5" /> {t('modal_title')}
            </DialogTitle>
            <DialogDescription>
              {t('modal_description')}
            </DialogDescription>
          ...
          <div>
            <label className="text-sm font-medium block mb-1.5">{t('modal_reason_label')}</label>
            <textarea
              value={killReason}
