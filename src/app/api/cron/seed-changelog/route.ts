...
const SEED_ENTRIES: Array<{
  ...
  {
    version: '0.12.0',
    title: '3 Strategi Inti + Adaptive Math Engine (drift content cleanup)',
    title_en: '3 Core Strategies + Adaptive Math Engine (drift content cleanup)',
    body: 'Editorial showcase dan copy site-wide direvisi untuk reflect backend reality:\n\n• "Enam strategi (SMC/Wyckoff/Astronacci/AI Momentum/Mean-Rev/Oil-Gas)" → "Tiga strategi inti + Adaptive Math Engine"\n• 3 strategi: SMC Scalper (Quasimodo family), SMC Swing H1-H4, Pivot Mean Reversion\n• Decision engine deterministic — rule-based router, Kelly sizing matematis, adaptive stop rule; AI advisor terpisah hanya untuk research/brief, tidak gate eksekusi\n• Subtitle "Lima pilar" → "Lima sorotan" supaya tidak konflik dengan platform 3-pillar\n\nSingle source of truth di backend market-substrate-api (VPS1:8220).',
    body_en: 'Editorial showcase and site-wide copy revised to reflect backend reality:\n\n• "Six strategies (SMC/Wyckoff/Astronacci/AI Momentum/Mean-Rev/Oil-Gas)" → "Three core strategies + Adaptive Math Engine"\n• 3 strategies: SMC Scalper (Quasimodo family), SMC Swing H1-H4, Pivot Mean Reversion\n• Deterministic decision engine — rule-based router, mathematical Kelly sizing, adaptive stop rule; separate AI advisor for research/brief only, not gating execution\n• Subtitle "Five pillars" → "Five highlights" to avoid conflict with platform 3-pillar\n\nSingle source of truth in backend market-substrate-api (VPS1:8220).',
    category: 'IMPROVEMENT',
...
export async function GET(req: Request) {
  ...
  //    Setelah ada minimum 1 entry, hanya strict mode yang allowed.
  //
  // Bootstrap allowed regardless of CRON_SECRET env value supaya CI/CD
  // post-deploy bisa populate /changelog di first deploy meskipun GH Actions
  // secret + container env beda timing. Setelah seed pertama success,
  // re-run akan reject (table not empty + no Bearer).
  const cronSecret = process.env.CRON_SECRET;
  const auth = req.headers.get('authorization') || '';
  const tokenMatch = !!cronSecret && auth === `Bearer ${cronSecret}`;
  const existingCount = await prisma.changelog.count();
  const bootstrapAllowed = existingCount === 0;

  if (!tokenMatch && !bootstrapAllowed) {
    return NextResponse.json({
      error: 'unauthorized',
      hint: 'Provide Bearer CRON_SECRET, atau table changelog kosong untuk bootstrap mode',
      tableEmpty: bootstrapAllowed,
    }, { status: 401 });
  }
