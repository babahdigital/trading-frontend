...
export function StrategiesSection({ t, localeKey, strategies }: StrategiesSectionProps) {
  return (
    <section className="section-padding border-b border-border/60">
      <div className="layout-container">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {strategies.map((s) => {
            const highlights = [1, 2, 3, 4, 5].map((n) => t(`strat_${s.slug}_h${n}`));
            return (
              <div key={s.slug} className="card-enterprise group">
                ...
                <p className="text-xs text-foreground/55 mb-3 leading-snug italic">{t(`strat_${s.slug}_tagline`)}</p>
                <div className="flex flex-wrap items-center gap-1.5 mb-4">
                  ...
                  <span className="text-[10px] font-mono uppercase tracking-wider px-1.5 py-0.5 rounded border border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-300">
                    {t(`strat_${s.slug}_tier`)}
                  </span>
                </div>
                <p className="t-body-sm text-foreground/65 leading-relaxed mb-4">{t(`strat_${s.slug}_desc`)}</p>
