...
/**
 * Table primitives — shadcn-style, token-driven untuk light/dark adapt.
 *
 * Pakai HSL tokens (var(--muted), var(--border)) supaya warna table konsisten
 * di kedua mode tanpa override manual per page. Wrapper `Table` punya
 * `overflow-x-auto` supaya mobile tidak push horizontal layout.
 *
 * Usage:
 *   <Table>
 *     <TableHeader>
...
const Table = React.forwardRef<HTMLTableElement, React.HTMLAttributes<HTMLTableElement> & { wrapperClassName?: string }>(
  ({ className, wrapperClassName, ...props }, ref) => (
    <div className={cn('relative w-full overflow-x-auto rounded-md border border-border bg-card', wrapperClassName)}>
      <table
        ref={ref}
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        'h-10 px-3 text-left align-middle font-semibold text-[11px] uppercase tracking-wider text-muted-foreground',
